---
title: Mickoblin
layout: default
permalink: /mickedex/mickoblin/
---

<figure style="float: right; margin: 0 0 20px 20px; width: 260px;">
  <img src="/assets/images/mick/mickoblin.png" alt="Mickoblin" style="width: 100%; border-radius: 8px;">
  <figcaption><strong>Mickoblin</strong><br>The Goblin Mickémon</figcaption>
</figure>

# Mickoblin

**Type:** Dark / Grass  
**Classification:** Goblin Mickémon  
**Mickédex Number:** #114  
**Height:** 2'00" (0.6 m)  
**Weight:**22.0 lbs (10.0 kg)  

### Short Mickédex Entry
"The Goblin Mickémon. It is a mischievous little goblin that loves causing trouble in the dark."

### Biology
Mickoblin is a small, green-skinned goblin Mickus with pointed ears, sharp teeth, and a mischievous grin. It wears a tattered cloak made from leaves and still has its signature mullet tuft poking out from under a tiny hood. It carries a small, jagged knife and loves playing pranks, stealing shiny objects, and setting traps in the dark. Despite its goblin nature, it retains the classic Mickus clumsiness, its traps often backfire on itself, and it frequently trips over its own cloak.

### Evolution
Evolves from Mickus while holding a Razor Fang and levelling at Night.  Evolves into Mickdemon at level 40, Mickgriffin if it levels while holding a Sharp Beak, Micktroll if it level while holding a Hard Stone, and Micklops if it levels while holding Soft Sand.

**Previous Evolution:** Mickus (Razor Fang)  
**Next Evolution(s):** Mickdemon (Level 40), Mickgriffin (Sharp Beak), Micktroll (Hard Stone), Micklops (Soft Sand)

### Gallery
![Mickoblin](/assets/images/mick/mickoblin.png){:width="30%" style="display: block; margin: 20px auto; border: 1px solid #ddd; border-radius: 8px;"}